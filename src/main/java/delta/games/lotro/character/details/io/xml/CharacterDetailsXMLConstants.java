package delta.games.lotro.character.details.io.xml;

/**
 * Constants for tags and attribute names used in the
 * character details XML files.
 * @author DAM
 */
public class CharacterDetailsXMLConstants
{
  /**
   * Tag 'characterDetails'.
   */
  public static final String CHARACTER_DETAILS_TAG="characterDetails";
  /**
   * Tag 'xp'.
   */
  public static final String XP_TAG="xp";
  /**
   * Tag 'xp', attribute 'current'.
   */
  public static final String CURRENT_XP_ATTR="current";
  /**
   * Tag 'currentTitle'.
   */
  public static final String CURRENT_TITLE_TAG="currentTitle";
  /**
   * Tag 'currentTitle', attribute 'id'.
   */
  public static final String CURRENT_TITLE_ID_ATTR="id";
  /**
   * Tag 'currentArea'.
   */
  public static final String CURRENT_AREA_TAG="currentArea";
  /**
   * Tag 'currentArea', attribute 'id'.
   */
  public static final String CURRENT_AREA_ID_ATTR="id";
  /**
   * Tag 'currentDungeon'.
   */
  public static final String CURRENT_DUNGEON_TAG="currentDungeon";
  /**
   * Tag 'currentDungeon', attribute 'id'.
   */
  public static final String CURRENT_DUNGEON_ID_ATTR="id";
  /**
   * Tag 'currentVocation'.
   */
  public static final String CURRENT_VOCATION_TAG="currentVocation";
  /**
   * Tag 'currentVocation', attribute 'id'.
   */
  public static final String CURRENT_VOCATION_ID_ATTR="id";
  /**
   * Tag 'characterDetails', attribute 'ingameTime'.
   */
  public static final String IN_GAME_TIME_ATTR="ingameTime";
  /**
   * Tag 'characterDetails', attribute 'money'.
   */
  public static final String MONEY_ATTR="money";
  /**
   * Tag 'characterDetails', attribute 'lastLogoutDate'.
   */
  public static final String LAST_LOGOUT_DATE_ATTR="lastLogoutDate";
  /**
   * Tag 'characterDetails', attribute 'noPurchaseRequired'.
   */
  public static final String NO_PURCHASE_REQUIRED_ATTR="noPurchaseRequired";
}
