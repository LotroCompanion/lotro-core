package delta.games.lotro.character.stats.base.io.xml;

/**
 * Constants for tags and attribute names used in the
 * XML persistence of basic stats sets.
 * @author DAM
 */
public class BasicStatsSetXMLConstants
{
  /**
   * Tag 'stats'.
   */
  public static final String STATS_TAG="stats";
  /**
   * Tag 'stat'.
   */
  public static final String STAT_TAG="stat";
  /**
   * Tag 'stat', attribute 'name'.
   */
  public static final String STAT_NAME_ATTR="name";
  /**
   * Tag 'stat', attribute 'operator'.
   */
  public static final String STAT_OPERATOR_ATTR="operator";
  /**
   * Tag 'stat', attribute 'value'.
   */
  public static final String STAT_VALUE_ATTR="value";
  /**
   * Tag 'stat', attribute 'descriptionOverride'.
   */
  public static final String STAT_DESCRIPTION_OVERRIDE_ATTR="descriptionOverride";
  /**
   * Tag 'stat', attribute 'modifiers'.
   */
  public static final String STAT_MODIFIERS_ATTR="modifiers";
}
