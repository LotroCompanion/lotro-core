...
public class LandblocksXMLWriter
{
  ...
  private void writeLandblock(TransformerHandler hd, Landblock landblock) throws SAXException
  {
    ...
    for(Integer cellIndex : cellIndexes)
    {
      ...
      Vector3D position=cell.getPosition();
      if (position!=null)
      {
        cellAttrs.addAttribute("","",LandblocksXMLConstants.CELL_X_ATTR,XmlWriter.CDATA,String.valueOf(position.getX()));
        cellAttrs.addAttribute("","",LandblocksXMLConstants.CELL_Y_ATTR,XmlWriter.CDATA,String.valueOf(position.getY()));
        cellAttrs.addAttribute("","",LandblocksXMLConstants.CELL_Z_ATTR,XmlWriter.CDATA,String.valueOf(position.getZ()));
      }
